...
for _ in range(generations):
    ...
    for i in range(1, len(new_population)):
        ...
        fit_ratio = this_fitness / best_fitness if best_fitness > 0 else 0.0
        exp_term = (math.exp(-k * fit_ratio) - math.exp(-k)) / (1 - math.exp(-k))
        mutation_rate = min_mutation + (max_mutation - min_mutation) * exp_term
        # mutation_rate = 0.25
    ...
    if get_fitness(population[0]) == str_len:
        break
